...
export const apiEditPost = async (post_id: number, data: IPostForm) => {
  const formData = new FormData();
  ...
  formData.append("address", data.address);
  formData.append("contact", data.contact);
  formData.append("id", String(post_id));
...
export async function apiGetDetailPost(id: number) {
  return await axios_client.get(`${_API_.FORUM.CREATE_POST}/${id}`); // Trả về dữ liệu từ API nếu cần
}
